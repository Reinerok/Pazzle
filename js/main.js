...
restart.addEventListener('click', () => {
    ...
    if (!tooltipImage.classList.contains('hide')) {
        hideAll(tooltipImage);
    }
    timerValue.classList.toggle('starting');
...
function shift(e) {
    ...
    if (e.target.classList.contains('direction')) {
        ...
        if (pieceOfPuzzle[item].style.order != 0 && pieceOfPuzzle[item].style.order <= complexity) {
            ...
            pieceOfPuzzle[item].style.order = pieceOfPuzzle[item].style.order < 1 ? 1 : pieceOfPuzzle[item].style.order;
            pieceOfPuzzle[item].style.order = pieceOfPuzzle[item].style.order > complexity ? complexity : pieceOfPuzzle[item].style.order;
            ...
            pieceOfPuzzle.forEach((element) => {
                ...
                compareString = compareString.length === stringToCompare.length ? '' : compareString;
            });
        }
    }
}